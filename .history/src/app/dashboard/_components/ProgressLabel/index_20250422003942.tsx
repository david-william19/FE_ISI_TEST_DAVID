...
export type Progress = 'ON_PROGRESS' | 'NOT_STARTED' | 'REJECT' | 'DONE'
...
export function ProgressLabel({progress}: ProgressLabelProps) {
    ...
    if(progress == 'ON_PROGRESS') {
        return (
            <span className={clsx(baseClass, 'bg-orange-500')}>
                On Progress
            </span>
        )
    }
    ...
    if(progress == 'REJECT') {
        return (
            <span className={clsx(baseClass, 'bg-red-500')}>
                Reject
            </span>
        )
    }
    ...
    return (
        <span className={clsx(baseClass, 'bg-slate-500')}>
            {progress}
        </span>
    )
} 