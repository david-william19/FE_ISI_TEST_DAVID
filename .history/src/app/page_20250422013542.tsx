...
export default function LoginPage() {
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center justify-center bg-[#FFDBDB]">
        {error && <div className="w-[400px] bg-red-400 text-white p-3 rounded-lg mb-2.5">{error}</div>}
        <div className="w-[400px] rounded-lg px-5 py-10 bg-white">
          ...
          {
            isLoading ? (
              <Button type="primary" className="min-w-[100px] mt-5">Please&nbsp;wait...</Button>
            ) : (
              <Button type="primary" className="w-[100px] mt-5" onClick={() => login({email, password})}>Sign in</Button>
            )
          }
          </div>
        </div>
